...
const DEFAULT_PAYEES: readonly [string, bigint][] = [
...
export type CreateValidatorUtxoResult = {
...
export type CreateValidatorUtxoInput = {
  fundingLovelace?: bigint;
  deadline?: bigint;
};

export async function createValidatorUtxo(
  input: CreateValidatorUtxoInput = {},
): Promise<CreateValidatorUtxoResult> {
  ...
  const fundingLovelace = input.fundingLovelace ?? runtime.fundingLovelace;

  const defaultDeadline = BigInt(Date.now()) + 24n * 60n * 60n * 1000n;
  const datumDeadline = input.deadline ?? defaultDeadline;
  const datum = makeEmptyOsiDatum(datumDeadline);

  for (const [pubKey, amount] of DEFAULT_PAYEES) {
  ...
  const txHash = await runtime.client
    ...
    .payToAddress({
      ...
      assets: makeLovelace(fundingLovelace),
  ...
  return {
    ...
    fundedLovelace: fundingLovelace.toString(),
    deadline: datum.deadline.toString(),
